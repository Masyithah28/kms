<?php
require 'koneksi.php'; // Make sure koneksi.php is included correctly
session_start();
...
$batas = 15;  // Number of records per page
$page = isset($_GET['p']) ? (int) $_GET['p'] : 1;
$startpage = ($page > 1) ? ($page * $batas) - $batas : 0;

// Ensure variables are passed as arguments
$data = getInstruksiData($startpage, $batas);
?>
...
<head>
    ...
    <title>Dokumen Instruksi - PT Petrokopindo Cipta Selaras</title>
...
<body>
      ...
      <a href="index-admin.php" class="nav-link"><i class="fas fa-home icon"></i>Master Dokumen</a>
        ...
        <div class="collapse" id="dokumenDropdown">
            <ul class="list-unstyled ms-3">
                ...
                <li><a href="index-dokumen3.php" class="dropdown-item active">Instruksi Kerja</a></li>
                ...
                <li><a href="index-dokumen8.php" class="dropdown-item">Kontrak / Perjanjian</a></li>
                <li><a href="index-dokumen7.php" class="dropdown-item">Lainnya</a></li>
            </ul>
        </div>
    ...
    <div class="content-container">
        ...
        <div class="header-container d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <h2 class="fw-bold mb-0">INSTRUKSI KERJA</h2>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                $batas = 15;  // Define how many records per page you want
                $page = isset($_GET['p']) ? (int) $_GET['p'] : 1;
                $startpage = ($page > 1) ? ($page * $batas) - $batas : 0;

                $result = getInstruksiData($startpage, $batas);
                ...
                if ($countData < 1) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin') {
                            ...
                            echo "<a href='#' class='btn btn-sm btn-action' style='background-color: #e74c3c; color: white;' title='Delete' onclick=\"return confirmDelete('aksiHapus.php?kodedelete={$row['id_berkas']}')\"><i class='fas fa-trash-alt'></i></a>";                             
                        }
                        echo "</td>"; 
                        echo "</tr>";
                    }
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="editKategori" name="kategori" required>
                                        ...
                                        <option value="Hasil Inovasi & Benchmarking">Hasil Inovasi & Benchmarking</option>
    ...
    <script>
        ...
        // Fungsi untuk mencari secara real-time di kategori Kebijakan
        document.getElementById('searchInput').addEventListener('keyup', function() {
            const query = this.value;

            // Buat permintaan AJAX ke search-kebijakan.php
            const xhr = new XMLHttpRequest();
            xhr.open('GET', 'search-instruksi.php?query=' + encodeURIComponent(query), true);

            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    // Ganti body tabel dengan hasil pencarian
        ...
        // Perform AJAX request to fetch sorted data from search_kebijakan.php
        const xhr = new XMLHttpRequest();
        const query = document.getElementById('searchInput').value; // Get current search input value
        xhr.open('GET', `search-instruksi.php?query=${encodeURIComponent(query)}&column=${encodeURIComponent(column)}&direction=${encodeURIComponent(newDirection)}`, true);
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (currentPath === "index-dokumen3.php") {
                const dokumenDropdown = document.querySelector("#dokumenDropdown");
                const instruksiLink = document.querySelector("a[href='index-dokumen3.php']");

                // Menambah kelas 'show' pada dropdown untuk tetap terbuka
                if (dokumenDropdown && instruksiLink) {
                    dokumenDropdown.classList.add("show");
                    instruksiLink.classList.add("active");
                }
            }
        });
    </script>
