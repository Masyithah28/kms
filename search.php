...
// Get the search query, sort column, and sort direction from GET parameters
$query = isset($_GET['query']) ? mysqli_real_escape_string($conn, $_GET['query']) : '';
$column = isset($_GET['column']) ? $_GET['column'] : 'id_berkas'; // Default sort column
$direction = isset($_GET['direction']) && $_GET['direction'] === 'desc' ? 'DESC' : 'ASC'; // Default sort direction

// Whitelist columns for safe sorting
$allowed_columns = ['id_berkas', 'nama_berkas', 'kategori', 'status', 'tgl_up', 'tgl_berlaku'];
if (!in_array($column, $allowed_columns)) {
    $column = 'id_berkas'; // Set to default column if invalid
}

// Build the SQL query to search and sort across all records
$sql = "SELECT * FROM tb_berkas WHERE 
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Other action buttons, e.g., download, edit, delete
        ...
        // Include edit and delete buttons for admin role
